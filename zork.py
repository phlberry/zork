...
def fuzzyFind():
    ...
    choice = pyfzf.FzfPrompt().prompt(set(dork_lists))
    return ''.join(choice)

# google search
def zorkin(query, userAgent, num):
    results = []
    for i in search(query, num=int(num), stop=int(num), pause=5, user_agent=userAgent):
        results.append(i)
    return results
...
def getOptions():
    ...
    if query_special['query']:
        query_blocks.append(query_special['query'])
...
def main():
    ...
    results = zorkin(opts['search_string'], opts['agent'], opts['num'])
